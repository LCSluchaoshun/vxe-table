...
export default {
  methods: {
    ...
    beginValidate(rows, cb, isFull) {
      const validRest = {};
      const { editRules, afterFullData, visibleColumn, treeConfig, treeOpts } =
        this;
      ...
      if (editRules) {
        ...
        const handleVaild = (row) => {
          if (isFull || !this.validRuleErr) {
            ...
            columns.forEach((column) => {
              if (
                (isFull || !this.validRuleErr) &&
                XEUtils.has(editRules, column.property)
              ) {
                colVailds.push(
                  this.validCellRules("all", row, column).catch(
                    ({ rule, rules }) => {
                      const rest = {
                        rule,
                        rules,
        ...
        if (treeConfig) {
          XEUtils.eachTree(vaildDatas, handleVaild, {
        ...
        return Promise.all(rowValids)
          ...
          .catch((firstErrParams) => {
            return new Promise((resolve, reject) => {
              ...
              const posAndFinish = () => {
                firstErrParams.cell = this.getCell(
                  firstErrParams.row,
              ...
              if (this.validOpts.autoPos === false) {
                ...
                const locatColumn =
                  columnIndex > 0 ? visibleColumn[rowIndex - 1] : column;
    ...
    validCellRules(validType, row, column, val) {
      ...
      if (property && editRules) {
        const rules = XEUtils.get(editRules, property);
        if (rules) {
          const cellValue = XEUtils.isUndefined(val)
    ...
    _clearValidate(rows, fieldOrColumn) {
      ...
      const colList = XEUtils.isArray(fieldOrColumn)
        ? fieldOrColumn
        : (fieldOrColumn ? [fieldOrColumn] : []).map((column) =>
            handleFieldOrColumn(this, column)
          );
      let validErrMaps = {};
